...
(define (mergeClusters L) ;input a global list
   ...
   (define (mergeClusters gl counter collector cl)                    
         ...
         (cond ((eqv? counter 0) cl)
               ...
               (else
                (if (eqv? (length collector) 0) (mergeClusters (rest gl) (- counter 1) (cons (list-ref (first gl) 5) collector)  (extractCluster((list-ref (first gl) 5) gl)))
                    ;else
                    (if (eqv? #f (member (list-ref (first gl) 5))) ;current cluster is not in the list
                        ...
                        (mergeClusters (rest gl) (- counter 1) (cons (list-ref (first gl) 5) collector) (intersection (extractCluster((list-ref (first gl) 5) gl)) cl))
